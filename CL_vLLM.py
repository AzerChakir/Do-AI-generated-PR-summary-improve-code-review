...
from utils import cl_prompt, ct_formatter, remove_diffs, count_matching_elements, calc_results, cl_prompt_summary, load_qa_dataframe
...
def prompt_combinations(example, mode, language_type, use_summary):
    ...
    if mode == "easy":
        options = [list(o) for o in example.loc_wrong_easy]
    elif mode == "hard":
        options = [list(o) for o in example.loc_wrong_hard]

    loc_correct = list(example.loc_correct)
    options.append(loc_correct)
    all_permutations = list(itertools.permutations(options))
    
    prompts = []
    correct_symbols = []
    for permutation in all_permutations:
        correct_symbols.append(symbol_index_map[permutation.index(loc_correct)])
        if use_summary:
