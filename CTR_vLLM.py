...
from utils import ctr_prompt, ct_formatter, remove_diffs, count_matching_elements, calc_results, ctr_prompt_summary, load_qa_dataframe
...
def prompt_combinations(example, language_type, use_summary):
    ...
    options = list(example.type_wrong)
...
### Evaluation
def test_example(example, tokenizer, llm, sampling_params, language_type, use_summary):
    symbols = ["A", "B", "C"]
...
def main():
    ...
    save_dir = f"{results_root}/ctr/{lang}/ctr_{lang}_{model_name_short}.pkl"
    ...
    check_gpu_memory(gpu_memory_utilization = 0.90)
    llm = LLM(model = model_name, tensor_parallel_size = torch.cuda.device_count(), max_model_len = 4000, gpu_memory_utilization = 0.90)
    ...
    for row in tqdm(range(len(mcqa_set))):
        example_save = test_example(mcqa_set.iloc[row], tokenizer, llm, sampling_params, language_type, use_summary)
